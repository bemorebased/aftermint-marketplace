...
export default function ListingStatusIndicator({ 
  ...
  if (status.status === 'loading') {
    return (
      <div className={`inline-flex items-center px-2 py-1 rounded-full text-xs ${className}`}>
        ...
        <span className="text-theme-text-secondary">Checking status...</span>
      </div>
    );
  }
  ...
  if (status.status === 'no-listing') {
    return (
      <div className={`inline-flex items-center px-2 py-1 rounded-full text-xs bg-theme-surface text-theme-text-secondary ${className}`}>
...
export function ListingActionButton({ 
  ...
  if (!isListed) {
    return (
      <button
        disabled
        className="w-full bg-theme-surface text-theme-text-secondary px-4 py-2 rounded cursor-not-allowed"
      >
        Not for sale
      </button>
    );
  ...
  return (
    <button
      ...
      className={`w-full px-4 py-2 rounded font-medium ${
        disabled
          ? 'bg-theme-surface text-theme-text-secondary cursor-not-allowed'
