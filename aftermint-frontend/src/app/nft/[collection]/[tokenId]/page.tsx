...
`);
  ...
  const handleConfirmListNft = async (listingDetails: {
    ...
    try {
      ...
      const listingTx = await listNFTForSale(
      ...
      console.log(`[NFTDetailPage] Listing transaction hash:`, listingTx.hash);
      ...
      if (receipt && receipt.status === 1) {
        ...
        // Refresh the listing state to update UI
        await refreshListingState();
        
      } else {
        toast.error('Transaction failed. Please try again.', { id: 'list-nft' });
      }
      ...
      if (error.message && error.message.includes('AlreadyListed')) {
        toast.error('This NFT is already listed. Refreshing listing status...', { id: 'list-nft' });
        // Refresh the listing state to show correct UI
        await refreshListingState();
        ...
        toast.error('Transaction cancelled by user', { id: 'list-nft' });
      } else if (error.message && error.message.includes('insufficient funds')) {
        toast.error('Insufficient funds for transaction fees', { id: 'list-nft' });
      } else {
        toast.error(`Failed to list NFT: ${error.message || 'Unknown error'}`, { id: 'list-nft' });
      }
    } finally {
  ...
  const fetchCollectionData = async () => {
    ...
    try {
      ...
      // Get collection offers using marketplace service (same pattern as collection page)
      const collectionListings = await getCollectionActiveListings(normalizedAddress, provider);
      
      // Convert listings to offers format for display
      const collectionOffersData = collectionListings.map((listing: any) => ({
        offerId: `${listing.tokenId}-${listing.seller}`, // Create unique ID
        from: listing.seller,
        to: '', // Not applicable for listings
        nftContract: normalizedAddress,
        tokenId: listing.tokenId.toString(),
        price: `${listing.price} BASED`,
        priceInEth: listing.price,
        expiration: listing.expiresAt && listing.expiresAt > 0 ? new Date(listing.expiresAt * 1000) : null,
        isExpired: listing.expiresAt && listing.expiresAt > 0 ? listing.expiresAt < Math.floor(Date.now() / 1000) : false,
      ...
      // Use marketplace ABI for events
      ...
      const saleFilter = marketplaceContract.filters.NFTSold(null, null, normalizedAddress);
      ...
      console.log(`[NFTDetailPage] Collection data fetched:`, {
        listings: collectionOffersData.length,
  ...
  // Check listing status on page load and when NFT data changes
  useEffect(() => {
    if (nft?.collection?.contract && tokenId && !loading) {
      console.log('[NFTDetailPage] Checking listing status on page load...');
      checkIfNFTIsListed();
    }
  }, [nft?.collection?.contract, tokenId, loading]);

  // Also check after any listing operation
  const refreshListingState = async () => {
    if (nft?.collection?.contract && tokenId) {
      await checkIfNFTIsListed();
    }
  };
  ...
  return (
    <div className="min-h-screen bg-theme-background text-theme-text-primary pb-20">
      ...
      <div className="container mx-auto px-4 py-6">
        <Link href="/collection" className="inline-flex items-center text-theme-text-secondary hover:text-theme-text-primary mb-6">
          ...
          <div className="flex flex-col lg:flex-row gap-8">
            ...
            <div className="w-full lg:w-1/2">
              <div className="mb-6">
                ...
                <h1 className="text-3xl font-bold mb-2">{nft.name || `NFT #${nft.tokenId}`}</h1>
