 ...
 */
export async function fetchUserNFTs(userAddress: string, limit: number = 50) {
  if (!userAddress || userAddress === 'undefined') {
    console.warn('Invalid user address provided to fetchUserNFTs');
    return [];
  }

  // Check cache first
  const cacheKey = `user_nfts_${userAddress.toLowerCase()}_${limit}`;
  const cached = explorerCache.get(cacheKey);
  if (cached && (Date.now() - cached.timestamp) < CACHE_DURATION) {
    console.log(`[fetchUserNFTs] Using cached data for ${userAddress}`);
    return cached.data;
  }

  try {
    console.log(`[fetchUserNFTs] Fetching NFTs for ${userAddress} from BasedAI explorer`);
    
    const response = await axios.get(`${EXPLORER_API_BASE}/v2/addresses/${userAddress}/nft`, {
      params: { limit }
    });
    
    if (response.data && response.data.items) {
      const nfts = response.data.items.map((nft: any) => ({
        contractAddress: nft.token?.address || null,
        tokenId: nft.token?.id || null,
        name: nft.token?.name || null,
        symbol: nft.token?.symbol || null,
        metadata: nft.metadata || null,
        image: nft.metadata?.image || null
      }));

      // Cache the result
      explorerCache.set(cacheKey, { data: nfts, timestamp: Date.now() });
      
      console.log(`[fetchUserNFTs] ✅ Found ${nfts.length} NFTs for ${userAddress}`);
      return nfts;
    }
    
    return [];
  } catch (error) {
    console.error(`[fetchUserNFTs] Error fetching NFTs for ${userAddress}:`, error);
    return [];
  }
}
