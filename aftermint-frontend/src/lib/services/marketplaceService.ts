...
export async function getListingInfo(
  ...
  try {
    ...
    // Use proper checksummed address instead of lowercase
    const checksummedAddress = ethers.getAddress(nftContractAddress);
    
    console.log(`[MarketplaceService] 🔍 Checking listing for ${checksummedAddress}#${tokenId}`);
    ...
    try {
      ...
      const listing = await marketplaceContract.getListing(checksummedAddress, tokenId);
      
      console.log(`[MarketplaceService] 📋 Raw listing data for ${checksummedAddress}#${tokenId}:`, {
      ...
      if (listing && listing.seller !== ethers.ZeroAddress) {
        ...
        if (isExpired) {
          console.log(`[MarketplaceService] 🕐 Listing for ${checksummedAddress}#${tokenId} has expired at ${new Date(Number(listing.expiresAt) * 1000).toLocaleString()}`);
          // Return null for expired listings so the UI knows it's not available
          return null;
        }
        
        console.log(`[MarketplaceService] ✅ Active listing found for ${checksummedAddress}#${tokenId}`);
      ...
      console.log(`[MarketplaceService] ❌ No listing found for ${checksummedAddress}#${tokenId} (seller is zero address)`);
      ...
      console.warn(`[MarketplaceService] Unexpected error getting listing for ${checksummedAddress}#${tokenId}:`, contractError.message || contractError);
      return null;
    }
  } catch (error) {
...
export async function listNFTForSale(
  ...
  try {
    console.log('[MarketplaceService] Starting listNFTForSale with params:', {
      nftContractAddress,
      tokenId,
      priceInBased,
      expirationDate,
    ...
    console.log(`[MarketplaceService] User address: ${userAddress}`);
    ...
    const priceInWei = typeof priceInBased === 'string' ? ethers.parseEther(priceInBased) : priceInBased;
    
    // Calculate expiration timestamp
    const expirationTimestamp = expirationDate ? Math.floor(expirationDate.getTime() / 1000) : 0;
    
    console.log(`[MarketplaceService] Processed parameters:`, {
    ...
    console.log('[MarketplaceService] Starting pre-flight checks...');
    
    // Check if NFT contract exists and is valid
    try {
      ...
      console.log(`[MarketplaceService] Token owner: ${tokenOwner}, User: ${userAddress}`);
      
      if (tokenOwner.toLowerCase() !== userAddress.toLowerCase()) {
        throw new Error(`You don't own token ${tokenId}. Owner: ${tokenOwner}, Your address: ${userAddress}`);
      }
      
      // Check if already listed
      const marketplaceContractReadOnly = getMarketplaceContract(provider);
      try {
        const existingListing = await marketplaceContractReadOnly.getListing(normalizedNftContract, tokenId);
        if (existingListing && existingListing.seller !== ethers.ZeroAddress) {
          throw new Error(`Token ${tokenId} is already listed. Please cancel the existing listing first.`);
        }
      } catch (error: any) {
        if (!error.message.includes('already listed')) {
          console.log('[MarketplaceService] No existing listing found (good)');
        } else {
          throw error;
        }
      }
      ...
      console.log(`[MarketplaceService] Approval status:`, {
        approvedAddress,
        isApprovedForAll,
        marketplaceAddress: MARKETPLACE_CONTRACT_ADDRESS
      });
      
      const isApproved = approvedAddress.toLowerCase() === MARKETPLACE_CONTRACT_ADDRESS.toLowerCase() || isApprovedForAll;
      
      if (!isApproved) {
        console.log('[MarketplaceService] NFT not approved, requesting approval...');
        ...
        try {
          // Try individual approval first
          const approveTx = await nftContractWithSigner.approve(MARKETPLACE_CONTRACT_ADDRESS, tokenId, {
            gasLimit: 100000
          });
          
          console.log('[MarketplaceService] Approval transaction sent:', approveTx.hash);
          ...
          console.log('[MarketplaceService] ✅ NFT approved successfully');
        } catch (approvalError: any) {
          console.error('[MarketplaceService] Approval failed:', approvalError);
          throw new Error(`Failed to approve NFT for marketplace: ${approvalError.message}`);
        }
      } else {
        console.log('[MarketplaceService] ✅ NFT already approved');
      }
      
    } catch (ownerError: any) {
      console.error(`[MarketplaceService] Pre-flight check failed:`, ownerError);
      throw new Error(`Pre-flight validation failed: ${ownerError.message}`);
    }

    // **STEP 3: List the NFT**
    console.log('[MarketplaceService] All pre-flight checks passed, proceeding with listing...');
    
    // Get the marketplace contract with signer
    const marketplaceContract = getMarketplaceContract(provider, signer);
    
    // Call the marketplace contract to list the NFT
    const listTx = await marketplaceContract.listNFT(
        normalizedNftContract, // Use normalized address
        tokenId,
        priceInWei,
        NATIVE_TOKEN_ADDRESS,  // address(0) for BASED
        expirationTimestamp,
    ...
    if (error.message) {
      if (error.message.includes('user rejected') || error.message.includes('User rejected')) {
        ...
        throw new Error('This NFT is already listed. Please cancel the existing listing first.');
      } else if (error.message.includes('not approved')) {
        throw new Error('NFT approval failed. Please try again or approve manually.');
      } else {
...
export async function getCollectionActiveListings(
  ...
  try {
    console.log(`[MarketplaceService] Scanning for active listings in collection ${collectionAddress}`);
    
    // Get the total supply to know the range of valid token IDs
    ...
    try {
      ...
      console.log(`[MarketplaceService] Collection ${collectionAddress} total supply: ${totalSupply}`);
    } catch (error) {
      console.warn(`[MarketplaceService] Could not get total supply, using fallback: ${totalSupply}`);
    }
    
    const listedTokenIds: number[] = [];
    const maxToCheck = Math.min(totalSupply, limit);
    
    // For better efficiency, check in batches and use smarter ranges
    // Check token IDs: 1 to maxToCheck (many NFT collections start from 1, not 0)
    const batchSize = 20;
    
    for (let batchStart = 1; batchStart <= maxToCheck; batchStart += batchSize) {
      const batchEnd = Math.min(batchStart + batchSize - 1, maxToCheck);
      console.log(`[MarketplaceService] Checking batch ${batchStart}-${batchEnd}...`);
      
      const batchPromises = [];
      for (let tokenId = batchStart; tokenId <= batchEnd; tokenId++) {
        batchPromises.push(
          getListingInfo(collectionAddress, tokenId, provider)
            .then(listing => {
    ...
    console.log(`[MarketplaceService] Found ${listedTokenIds.length} active listings for collection ${collectionAddress}: [${listedTokenIds.slice(0, 10).join(', ')}${listedTokenIds.length > 10 ? '...' : ''}]`);
