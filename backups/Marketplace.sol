    ...
    ReentrancyGuardUpgradeable
{
    ...
    function _initializeSubscriptionTiers() internal {
        // Tier 0: 7 days, 1 $BASED (1e18 wei), 0 bps fee
        ...
        emit SubscriptionTierAdded(0, 1 ether, address(0), 7 days, 0, true);

        // Tier 1: 30 days, 7 $BASED (7e18 wei), 0 bps fee
        subscriptionTiers.push(SubscriptionTier({
            price: 7 ether, // 7e18
            paymentToken: address(0),
        ...
        emit SubscriptionTierAdded(1, 7 ether, address(0), 30 days, 0, true);

        // Tier 2: 365 days, 30 $BASED (30e18 wei), 0 bps fee
        subscriptionTiers.push(SubscriptionTier({
            price: 30 ether, // 30e18
            paymentToken: address(0),
            durationSeconds: 365 days,
            feeBps: 0,
            isActive: true
        }));
        emit SubscriptionTierAdded(2, 30 ether, address(0), 365 days, 0, true);
    }
    ...
    function listNFT(
        ...
        uint256 tokenId,
        uint256 price,
        address paymentToken, // In Phase 1, this MUST be address(0)
        ...
        uint256 storageIndex = marketplaceStorage.recordListing(
            nftContract,
            tokenId,
            msg.sender,
            price,
        ...
        listings[nftContract][tokenId] = Listing({
            ...
            historicalListingIndex: storageIndex // Store the index
        });
        ...
        // Note: Call to marketplaceStorage.recordListing is now done before creating the local listing
     ...
     *      Updates the corresponding historical listing in MarketplaceStorage as cancelled.
     * @param nftContract The address of the NFT contract.
     * @param tokenId The ID of the token in the listing to be cancelled.
     */
    function cancelListing(
        ...
        uint256 storageIndex = currentListing.historicalListingIndex;

        delete listings[nftContract][tokenId];

        emit ListingCancelled(nftContract, tokenId, currentListing.seller);

        // Call MarketplaceStorage to update the listing status
        if (address(marketplaceStorage) != address(0)) {
            marketplaceStorage.updateListingStatus(storageIndex, MarketplaceStorage.ListingStatus.Cancelled, uint64(block.timestamp)); 
        } else {
            // This case should ideally not be reached if marketplaceStorage is set during initialization
            // and is not changeable to address(0) without explicit admin action (which we don't have yet).
            // However, if it could be unset, this implies an operational issue.
            // For now, we assume it's always set. A revert here might be too strict if temporary unsetting is possible.
        }
    }
    ...
    function updateListingPrice(
        ...
        // Ensure the listing is not expired if we want to prevent updates to expired listings
    ...
    function buyNFTNative(
        ...
        if (currentListing.privateBuyer != address(0) && currentListing.privateBuyer != msg.sender) {
        ...
        if (msg.value != currentListing.price) {
            revert Marketplace__WrongNativeValueSent(currentListing.price, msg.value);
        }

        // --- Gather Payment Details ---
        address seller = currentListing.seller;
        uint256 price = currentListing.price;
        address paymentToken = currentListing.paymentToken; // Will be address(0)
    ...
    function cancelOffer(
        address nftContract,
        uint256 tokenId
    ) public whenNotPaused nonReentrant { // nonReentrant to protect escrow changes
        Offer memory currentOffer = offers[nftContract][tokenId][msg.sender];

        if (currentOffer.bidder == address(0)) {
            revert Marketplace__OfferNotFound(nftContract, tokenId, msg.sender);
        }

        uint256 amountToRefund = currentOffer.amount;
        address paymentToken = currentOffer.paymentToken;
        uint256 storageIndex = currentOffer.historicalOfferIndex; // Get the stored index

        delete offers[nftContract][tokenId][msg.sender];

        if (paymentToken == address(0) && amountToRefund > 0) { // Native $BASED offer
            if (escrowedForOffers[msg.sender] < amountToRefund) {
                 revert Marketplace__InconsistentEscrowBalance(amountToRefund, escrowedForOffers[msg.sender]); 
            }
            escrowedForOffers[msg.sender] -= amountToRefund;
            Address.sendValue(payable(msg.sender), amountToRefund);
    ...
    function acceptOffer(
        ...
        if (activeListing.seller == msg.sender) { // Check if the seller (current owner) had it listed
            ...
            delete listings[nftContract][tokenId];
            emit ListingCancelled(nftContract, tokenId, msg.sender); // Emit cancellation for the listing
            if (address(marketplaceStorage) != address(0)) {
     ...
     * @param nftContract The address of the NFT contract collection.
     * @param tokenIds An array of token IDs to purchase from the given collection.
     * @param maxTotalPrice The maximum total price the user is willing to pay for the entire batch.
     */
    function buyMultipleNFTs(
        ...
        if (numItems > MAX_BATCH_BUY_COUNT) {
        ...
        if (calculatedTotalPrice > maxTotalPrice) {
            revert Marketplace__BatchBuyPriceExceedsMax(calculatedTotalPrice, maxTotalPrice);
        }
        if (msg.value != calculatedTotalPrice) {
            revert Marketplace__BatchBuyTotalPriceMismatch(calculatedTotalPrice, msg.value);
        }
     ...
     */
    function purchaseSubscription(uint256 tierId) public payable whenNotPaused nonReentrant {
        if (tierId >= subscriptionTiers.length) {
            revert Marketplace__InvalidSubscriptionTierId(tierId);
        }
        ...
        if (!tier.isActive) {
            revert Marketplace__SubscriptionTierNotActive(tierId);
        }

        // For Phase 1: Assume payment is in native $BASED
    ...
    function reclaimExpiredOfferEscrow(
        ...
        if (currentOffer.expiresAt == 0 || currentOffer.expiresAt >= block.timestamp) {
            revert Marketplace__OfferNotExpired(currentOffer.expiresAt);
        }

        uint256 amountToReclaim = currentOffer.amount;
        address paymentToken = currentOffer.paymentToken; // Should be address(0) for $BASED
        uint256 historicalOfferIndex = currentOffer.historicalOfferIndex;
