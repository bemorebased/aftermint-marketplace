    ...
    ReentrancyGuardUpgradeable
{
    ...
    function updateListingStatus(
        ...
        require(listingIndex < historicalListings.length, "Invalid listing index");
        
        HistoricalListing storage listing = historicalListings[listingIndex];
        listing.status = newStatus;

        if (newStatus == ListingStatus.Cancelled) {
            listing.cancelledAt = actionTimestamp;
        } else if (newStatus == ListingStatus.Sold) {
            listing.soldAt = actionTimestamp;
        } else if (newStatus == ListingStatus.Expired) {
            // For now, Marketplace.sol handles expiration logic without calling storage to update status to Expired.
            // If explicit "Expired" status update from Marketplace.sol is needed later, this can be used.
            // listing.expiresAt should already be set. We could add an explicitlyExpiredAt timestamp if needed.
        }
        // Ensure other timestamps are not reset here unless that's the desired logic,
        // e.g., a listing marked Sold shouldn't have its soldAt timestamp cleared if later marked something else (unlikely scenario).
    ...
    function updateOfferStatus(
        ...
        if (newStatus == OfferStatus.Cancelled) {
            ...
            // This status could be set if Marketplace.sol explicitly calls to mark an offer as Expired in storage.
